...
Owner: Eric / Aurora
Repo: `outsourc-e/fullstack-aisquad`
Scope: HermesWorld inside FullStack AiSquad, dashboard/plugin embedded first, standalone later

## Product thesis

HermesWorld is no longer a novelty route. It is the playable layer for FullStack AiSquad: a persistent world where humans and agents can move, talk, complete missions, unlock progression, and eventually keep working while the human is away.

Keep HermesWorld in `fullstack-aisquad` for now. The tight coupling to workspace state, sessions, agents, plugins, quests, and dashboard embeds is the feature. A standalone destination can ship as a route/deploy target later without splitting source.
...
- Dashboard plugin and FullStack AiSquad plugin remain first-class.
