...
class TestToolUseEnforcementConfig:
    ...
    def test_auto_skips_for_claude(self):
        from agent.prompt_builder import TOOL_USE_ENFORCEMENT_GUIDANCE
        agent = self._make_agent(model="gemma4:26b", tool_use_enforcement="auto")
        prompt = agent._build_system_prompt()
        assert TOOL_USE_ENFORCEMENT_GUIDANCE not in prompt

    def test_true_forces_for_all_models(self):
        from agent.prompt_builder import TOOL_USE_ENFORCEMENT_GUIDANCE
        agent = self._make_agent(model="gemma4:26b", tool_use_enforcement=True)
        prompt = agent._build_system_prompt()
        assert TOOL_USE_ENFORCEMENT_GUIDANCE in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_config_default_openrouter(self, agent):
        ...
        agent.provider = "openrouter"
        agent.base_url = "http://localhost:11434/v1"
        agent.model = "gemma4:26b-20250514"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
...
class TestRunConversation:
    ...
    def test_empty_response_triggers_fallback_provider(self, agent):
        ...
        agent._fallback_chain = [{"provider": "openrouter", "model": "gemma4:26b"}]
...
class TestMaxTokensParam:
    ...
    def test_not_tricked_by_openai_in_openrouter_url(self, agent):
        agent.base_url = "http://localhost:11434/v1/api.openai.com"
        result = agent._max_tokens_param(4096)
        assert result == {"max_tokens": 4096}
...
class TestGpt5ApiModeRouting:
    ...
    def test_is_azure_openai_url_detection(self, agent):
        ...
        assert agent._is_azure_openai_url("http://localhost:11434/v1") is False
...
class TestFallbackAnthropicProvider:
    ...
    def test_fallback_to_openrouter_uses_openai_client(self, agent):
        agent._fallback_activated = False
        agent._fallback_model = {"provider": "openrouter", "model": "gemma4:26b"}
        agent._fallback_chain = [agent._fallback_model]
        agent._fallback_index = 0
