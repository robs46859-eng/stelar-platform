...
def _mock_resolve(base_url="http://localhost:11434/v1", api_key="fallback-key-1234"):
...
class TestRestorePrimaryRuntime:
    ...
    def test_restores_model_and_provider(self):
        agent = _make_agent(
            fallback_model={"provider": "openrouter", "model": "gemma4:26b"},
        )
...
class TestTryRecoverPrimaryTransport:
    ...
    def test_skipped_for_openrouter(self):
        agent = _make_agent(provider="openrouter", base_url="http://localhost:11434/v1")
        error = _make_transport_error("ReadTimeout")
