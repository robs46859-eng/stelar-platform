...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("HERMES_INFERENCE_MODEL", " gemma4:26b\n")

    assert server._resolve_model() == "gemma4:26b"
    ...
    resp = server.handle_request(
        {
            ...
            "params": {
                "session_id": "sid",
                "key": "model",
                "value": "gemma4:26b --global",
            },
        }
    )

    assert resp["result"]["value"] == "gemma4:26b"
    assert seen["is_global"] is True
    assert saved["model"]["default"] == "gemma4:26b"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    try:
        resp = server.handle_request(
            {
                ...
                "params": {
                    "session_id": "sid",
                    "key": "model",
                    "value": "gemma4:26b --provider anthropic",
                },
            }
        )
        ...
        assert os.environ["HERMES_MODEL"] == "gemma4:26b"
        assert os.environ["HERMES_INFERENCE_MODEL"] == "gemma4:26b"
    finally:
        server._sessions.clear()
