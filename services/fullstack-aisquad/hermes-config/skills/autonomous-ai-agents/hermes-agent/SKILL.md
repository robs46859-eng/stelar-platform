...
- **Persistent cross-session memory** — remembers who you are, your preferences, environment details, and lessons learned. Pluggable memory backends (built-in, Honcho, Mem0, and more) let you choose how memory works.
...
The in-browser Web UI runs as a FastAPI/uvcorn server exposed by the `hermes dashboard` command. See the dedicated guide in `references/dashboard-web-ui.md` for access, reverse-proxy setup, CORS patching, and `--insecure` binding.
...
of differences between Win32 and POSIX have bitten us — document new ones here
so the next person (or the next session) doesn't rediscover them from scratch.
...
- Winsock / Windows-specific regression tests — `@pytest.mark.skipif(sys.platform != "win32", ...)`
...
### Browser tool fails on Azure VMs / cloud containers (Chrome "No usable sandbox")
The `browser_navigate` tool fails with `FATAL:content/browser/zygote_host/zygote_host_impl_linux.cc:128] No usable sandbox!` on Azure VMs, containers, and any Linux environment where AppArmor restricts user namespaces. Fix:

1. Add `--no-sandbox` flags to `~/.hermes/config.yaml` under `browser.args`:
```yaml
browser:
  args:
  - --no-sandbox
...
2. **The config change requires a session restart.** Run `/new` to pick up the new browser args. The current session will NOT apply browser config changes mid-conversation. Exporting `CHROMIUM_FLAGS="--no-sandbox --disable-setuid-sandbox --disable-dev-shm-usage"` to `.bashrc` alone does NOT work — the `config.yaml` `browser.args` is the reliable path.

3. **Verify Chrome is installed:** `which google-chrome-stable google-chrome chromium-browser chromium 2>/dev/null`. If none of these return a path, Chrome is not on the system and the browser tool can't work regardless of sandbox config.

### Web dashboard not visible in browser
The dashboard defaults to `http://127.0.0.1:9119` (localhost-only). It won't be reachable from another machine or via a domain name until you either:
- **Reverse-proxy** it through nginx/Caddy (recommended — keeps the dashboard bound to localhost), or
- **Bind to all interfaces** with `hermes dashboard --host 0.0.0.0 --insecure` (⚠️ exposes API keys on the network; use only on trusted LAN/VPN).

In both cases, the built-in CORS policy (`allow_origin_regex` in `web_server.py`) only permits `localhost` and `127.0.0.1` origins. If you access the dashboard through a domain name or reverse proxy, you must add that domain to the CORS regex or requests will be blocked by the browser.

Full details and recipes: see `references/dashboard-web-ui.md` in this skill.
...
**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The Hermes-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site, then run tests directly:
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")`
- `signal.SIGALRM` → Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
