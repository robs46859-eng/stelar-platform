...
> **Audience:** AI agents helping users set up FullStack AiSquad.
...
```
┌─────────────────┐       HTTP :8642        ┌──────────────────┐
│  Claude Gateway  │◄──────────────────────►│ FullStack AiSquad │
...
```bash
# In the fullstack-aisquad directory
...
```bash
cd ~/fullstack-aisquad   # or wherever it's installed
pnpm dev
```
...
# 6. Set workspace env
cd ~/fullstack-aisquad 2>/dev/null || cd "$(find ~ -maxdepth 2 -name fullstack-aisquad -type d | head -1)"
...
echo "=== workspace .env ===" && grep CLAUDE ~/fullstack-aisquad/.env 2>&1 || echo "no .env"
echo "=== OS ===" && uname -a
