...
FullStack AiSquad + Hermes Agent in containers.

## TL;DR (single-host, localhost-only)

```bash
git clone https://github.com/outsourc-e/fullstack-aisquad
...
- `fullstack-aisquad` (port `3000`, bound to `127.0.0.1`)
...
```bash
docker compose exec fullstack-aisquad sh
...
```bash
docker compose exec fullstack-aisquad env | grep -E "HERMES_API|API_SERVER"
```
...
```bash
docker compose logs fullstack-aisquad 2>&1 | grep '\[gateway\]' | tail -3
```
...
## FullStack AiSquad + Hermes Agent: why two containers?

The workspace is the **UI**. The agent is the **engine**. Splitting them lets you:

- Update either independently (`docker compose pull fullstack-aisquad` etc.)
...
If your setup matches the playbook above and still breaks, file an issue at <https://github.com/outsourc-e/fullstack-aisquad/issues> with:

1. Your `docker-compose.yml` (redact secrets)
2. The output of `docker compose logs fullstack-aisquad 2>&1 | grep '\[gateway\]' | tail -5`
