  ...
  properties: {
    ...
    template: {
      containers: [{
        ...
        probes: [
          { type: 'Startup', httpGet: { path: '/health', port: 3100 }, initialDelaySeconds: 5, failureThreshold: 30, periodSeconds: 5 }
          { type: 'Liveness', httpGet: { path: '/health', port: 3100 }, initialDelaySeconds: 15 }
          { type: 'Readiness', httpGet: { path: '/ready', port: 3100 }, initialDelaySeconds: 10 }
        ]
      }]
